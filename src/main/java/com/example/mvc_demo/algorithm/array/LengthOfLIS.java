...
public class LengthOfLIS {
    ...
    public int lengthOfLIS(int[] arr) {
        ...
        for (int i = 1; i < arr.length; i++) {
            if (arr[i] < nums[index]) {
                int needRep = indexOfRep(nums, arr[i], 0, index);
                nums[needRep] = arr[i];
            } else if (arr[i] > nums[index]) {
                nums[++ index] = arr[i];
            }
        }
    ...
    public int indexOfRep(int[] nums, int target, int start, int end) {
        ...
        if (nums[middle] == target) {
            ...
            if (middle == 0) {
                ...
                return indexOfRep(nums, target, start, middle - 1);
            }
        } else {
            return indexOfRep(nums, target, middle + 1, end);
        }
    }
